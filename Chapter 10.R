...
mom_rank = KOR_ticker[invest_mom, ] %>%
  select(`종목코드`, `종목명`) %>%
...
sector_neutral = data_market %>%
  ...
  mutate(scale_per_sector = scale(`ret`),
         scale_per_sector = ifelse(is.na(`SEC_NM_KOR`),#scale함수 이용해 그룹별 정규화
...
#1월~4월의 경우 전년도 재무제표가 일부만 들어오는 경향이 있으므로, 
...
magic_debt = KOR_fs$'부채'[num_col]   #총부채는 부채 항목
magic_excess_cash_1 = KOR_fs$'유동부채' - KOR_fs$'유동자산' +  KOR_fs$'현금및현금성자산'   # 순운전자본
...
##10.3.2 윈저라이징(Winsorizing): 이상치 데이터 대체(포트폴리오 구성에서는 일반적으로 이상치 데이터를 다른 데이터로 대체하는 윈저라이징 방법이 사용)
...
#그러나 전문투자자의 입장이거나 팩터를 분석하는 업무를 할 경우 단순 랭킹 더하는 방법은 문제 안고 있음
